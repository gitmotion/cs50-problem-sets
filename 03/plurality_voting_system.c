...
bool vote(string name)
{
    // check if candidate exists in the candidates array
    for (int i = 0; i < candidate_count; i++)
    {
        if (strcmp(candidates[i].name, name) == 0)
        {
            // if so update/increment their vote
            candidates[i].votes++;
            return true;
        }
    }
...
void print_winner(void)
{
    ...
    // if there is a tie then both names should be printed on separate lines
    for (int i = 0; i < candidate_count; i++)
    {
